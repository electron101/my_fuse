...
static int memfs_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    // Проверка на права доступа на запись (сначала через o_flags)
    if (!(fh->o_flags & O_WRONLY) && !(fh->o_flags & O_RDWR)) {
        return -EACCES;  // Нет прав на запись
    }

    // Проверка прав на запись (mode) на уровне файловой системы
    if (!(node->vstat.st_mode & S_IWUSR)) {  // Проверка прав записи для владельца
        return -EACCES;  // Нет прав на запись
    }

    // Вычисление количества необходимых блоков
    ...
    if (node->vstat.st_blocks < req_blocks) {
        ...
        if (!newdata) {
            return -ENOMEM;  // Не удалось выделить память
        }
        ...
        // Обновляем информацию об аллокации
        node->data = newdata;
        node->vstat.st_blocks = req_blocks;
    }

    // Записываем данные в буфер файла
    memcpy(((char *) node->data) + offset, buf, size);

    // Обновляем размер файла, если необходимо
    off_t new_size = offset + size;
    if (new_size > node->vstat.st_size) {
        node->vstat.st_size = new_size;
    }
